...
local packer = Augists.utils.plugins.require('packer')

packer.init(Augists.plugins.packer.init)

local function is_enabled(plugin)
    return Augists.plugins[plugin].enabled
end

local function get_cmp()
    if Augists.plugins.cmp.enabled == true then
        return 'nvim-cmp'
    else
        return
    end
...
return packer.startup(function(use)
    ...
    -- use {'akinsho/nvim-bufferline.lua', requires = 'kyazdani42/nvim-web-devicons', config = 'require"bufferline-config"'}
    use {
    ...
    for _, plugin in pairs(Augists.plugins.user) do use(plugin) end
end)

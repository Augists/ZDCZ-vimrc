...
local require_plugin = function(p)
    if Augists.plugins[p] ~= nil and not Augists.plugins[p].enabled then return nil end
...
local add_which_key = function(starter_key, definitions)
    Augists.plugins.which_key.user_defined[starter_key] = definitions or {}
end
...
Augists.utils = {
        ...
        copy = function(t1, t2)
            for k, v in pairs(t2) do
                if (type(v) == "table") and (type(t1[k] or false) == "table") then
                    Augists.utils.tables.copy(t1[k], t2[k])
                else
                    t1[k] = v
                end
            end
    ...
    plugins = {
        ...
        add_user = function(p)
            table.insert(Augists.plugins.user, p)
        end,
        -- Allows us to require packages in augists-user-config
        -- without throwing exceptions if the package don't exist
        -- Optionally you can run this like some/package to add it
        -- to the Augists.packages.user table for installation.
        --
        ...
        require_if_installed = function(mod_name, pkg, conf)
            ...
            elseif pkg_is_git then
                table.insert(Augists.plugins.user, {packer_ref, conf})
                return nil
            end
        end
    }

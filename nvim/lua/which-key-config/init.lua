Augists.utils.plugins.packadd('which-key.nvim')

local wk = Augists.utils.plugins.require('which-key')
...
if Augists.plugins.nvim_tree.enabled then mappings.e = {":NvimTreeToggle<cr>", "File Explorerer"} end

if Augists.plugins.dashboard.enabled then mappings.d = {":Dashboard<cr>", "Dashboard"} end

if Augists.plugins.telescope.enabled then
    mappings.f = {
        name = "Telescope",
...
if Augists.plugins.nvim_comment.enabled then mappings["/"] = {":CommentToggle<cr>", "Toggle Comment"} end

if not Augists.settings.always_force_write then
    mappings.W = {":w!<cr>", "Force Write"}
else
    -- map n mode w to w!
end

for plugin, plugin_options in pairs(Augists.plugins) do
...
mappings = Augists.utils.tables.copy(mappings, Augists.plugins.which_key.user_defined)

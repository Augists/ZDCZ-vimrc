Augists.utils.plugins.packadd('nvim-lspconfig')

local lspconfig = Augists.utils.plugins.require('lspconfig')

local capabilities = require('cmp_nvim_lsp').update_capabilities(vim.lsp.protocol.make_client_capabilities())
capabilities.textDocument.completion.completionItem.snippetSupport = true

-- Language servers
for ls_type, props in pairs(Augists.language_servers) do
...
elseif Augists.language_servers.sumneko_lua['root_path'] ~= nil then
    sumneko_root_path = Augists.language_servers.sumneko_lua.root_path
    sumneko_binary = Augists.language_servers.sumneko_lua.binary_path
else
    print("Unsupported system for sumneko")
end

if Augists.language_servers.sumneko_lua.enabled and sumneko_binary ~= "" and not Augists.utils.file.exists(sumneko_binary) then
    print('Unable to load Sumneko language server.  Make sure it is installed in ' .. sumneko_root_path)
else
    local luadev = Augists.utils.plugins.require('lua-dev')
...
if Augists.language_servers.html.enabled then
